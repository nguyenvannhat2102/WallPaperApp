...
public class CuratedAdapter extends RecyclerView.Adapter<CurateViewHolder> {
    ...
    @NonNull
    @Override
    public CurateViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new CurateViewHolder(LayoutInflater.from(context).inflate(R.layout.home_list,parent,false));
    }

    @Override
    public void onBindViewHolder(@NonNull CurateViewHolder holder, int position) {
        Picasso.get().load(list.get(position).getSrc().getMedium()).placeholder(R.drawable.loading).into(holder.imageView_list);
...
class CurateViewHolder extends RecyclerView.ViewHolder{

    CardView home_list_container;
    ImageView imageView_list;
    public CurateViewHolder(@NonNull View itemView) {
        super(itemView);
